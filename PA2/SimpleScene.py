...
def set_cow_rotation(dir_vec):
    global cow_rotation_pos

    roll = 0
    pitch = np.arcsin(dir_vec[1])
    yaw = np.arctan2(dir_vec[2], dir_vec[0])
    ...
    Rx = np.array([[1, 0, 0],
                  [0, np.cos(pitch_temp), -np.sin(pitch)],
                  [0, np.sin(pitch), np.cos(pitch_temp)]])
    ...
    cow_rotation_pos[:3, :3] = (Ry@Rx@Rz).T
...
def display():
    global cameraIndex, cow2wld, input_count, input_location, anim_start_time, input_done, cow_rotation_pos, animating
    ...
    if -1 < input_count and input_count < 6:
        for i in input_location:
            drawCow(i, False)
    elif input_count == 6:
        animating = True
        anim_time = glfw.get_time() - anim_start_time

        cow_spline_pos = np.zeros((4, 4))

        cnt = int(anim_time) % 6

        if cnt < 6 and anim_time < 18:
            cow_spline_pos = catmull_rom_spline(
                input_location[(cnt - 1) % 6], 
                input_location[cnt], 
                input_location[(cnt + 1) % 6], 
                input_location[(cnt + 2) % 6],
                anim_time - int(anim_time))

        else:
            cow2wld = cow_rotation_pos
            input_count = -1
        ...
        set_cow_rotation(normalize(getTranslation(cow_spline_pos) - getTranslation(cow_rotation_pos)))
        setTranslation(cow_rotation_pos, getTranslation(cow_spline_pos))
        drawCow(cow_rotation_pos, False)
...
def initialize(window):
    global cursorOnCowBoundingBox, floorTexID, cameraIndex, camModel, cow2wld, cowModel, cow_rotation_pos
    cursorOnCowBoundingBox=False;
    # Set up OpenGL state
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, input_count, anim_start_time, input_done, cow_rotation_pos
    GLFW_DOWN=1;
    GLFW_UP=0;

    # animation이 진행중일 때는 좌클릭 무시
    if animating:
        return
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if cursorOnCowBoundingBox and not animating:
                input_count += 1
                if input_count > 0:
                    input_location.append(cow2wld.copy())
                    if input_count == 6:
                        anim_start_time = glfw.get_time()
                        input_done = True
                        cow_rotation_pos = input_location[0].copy()
                        isDrag = 0
...
def onMouseDrag(window, x, y):
    global isDrag,cursorOnCowBoundingBox, pickInfo, cow2wld, input_count, animating
    if isDrag: 
